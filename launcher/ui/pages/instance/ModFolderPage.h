...
class ModFolderPage : public ExternalResourcesPage {
   ...
   private:
    QStringList selectedFolderNames() const;
    QString selectedFolderNameForRename() const;
    bool commitModFolders(ModFolderStorage&& candidate);

   protected:
    ModFolderModel* m_model;
    QPointer<ResourceDownload::ModDownloadDialog> m_downloadDialog;

    std::unique_ptr<ModFolderStorage> m_folderStorage;
    ModFolderProxyModel* m_folderProxy = nullptr;
