...
void ExternalResourcesPage::setViewModel(QAbstractItemModel* model)
{
    m_viewModel = model;
...
QModelIndex ExternalResourcesPage::mapViewToSource(const QModelIndex& index) const
{
    if (!index.isValid()) {
        return {};
    }
    ...
    const QAbstractItemModel* currentModel = m_viewModel;
    while (currentModel && currentModel != m_model) {
        const auto* proxy = qobject_cast<const QAbstractProxyModel*>(currentModel);
        if (!proxy) {
            return {};
        }
    ...
    return currentModel == m_model ? mappedIndex : QModelIndex();
}

QModelIndexList ExternalResourcesPage::mapViewIndexesToSource(const QModelIndexList& indexes) const
{
    QModelIndexList mappedIndexes;
    mappedIndexes.reserve(indexes.size());
    for (const auto& index : indexes) {
        const auto mappedIndex = mapViewToSource(index);
        if (mappedIndex.isValid()) {
            mappedIndexes.append(mappedIndex);
        }
    }
    return mappedIndexes;
}

QItemSelection ExternalResourcesPage::mapViewSelectionToSource(const QItemSelection& selection) const
{
    QItemSelection mappedSelection;
    for (const auto& index : selection.indexes()) {
        const auto mappedIndex = mapViewToSource(index);
        if (mappedIndex.isValid()) {
            mappedSelection.select(mappedIndex, mappedIndex);
        }
    }
...
{
    auto selection = mapViewSelectionToSource(ui->treeView->selectionModel()->selection());
...
void ExternalResourcesPage::updateActions()
{
    ...
    const QModelIndexList selection = mapViewSelectionToSource(ui->treeView->selectionModel()->selection()).indexes();
